...
export function logicPage () {
  ...
  const body = wrap(`
    ...
    <h2 style="margin-top:var(--space-9)">Condition trees</h2>
    <div class="card">${renderValue(o.condition_ast)}</div>

    <h2 style="margin-top:var(--space-9)">Decision shape</h2>
    <div class="card">${renderValue(o.decision_shape)}</div>

    <h2 style="margin-top:var(--space-9)">Action vocabulary</h2>
    ...
    <h2 style="margin-top:var(--space-9)">The rule schema</h2>
  ...
  const table = (rows, cols, caption) => `
    <div class="table-scroll">
      <table>
        ...
        <tbody>${rows.map((r) => `<tr>${cols.map((c) => `<td${c.strong ? ' style="color:var(--rp-heading);font-weight:600"' : ''}>${esc(typeof r === 'object' ? r[c.key] ?? '' : r)}</td>`).join('')}</tr>`).join('')}</tbody>
      </table>
    </div>`
...
export function templatesPage () {
  const body = wrap(`
    ...
    ${corpus.templates.map((t) => `
      <article class="panel" id="${attr(t.id)}" style="margin-top:var(--space-6)">
        <p class="mono muted" style="font-size:13px;margin:0 0 var(--space-2)">${esc(t.id)}</p>
        <h2 style="font-size:28px">${esc(t.name)}</h2>
        <div class="grid grid--2" style="margin-top:var(--space-5)">
          <div>
            <p class="eyebrow" style="margin-bottom:var(--space-3)">Use when</p>
            ${conditionTree(t.use_when)}
          </div>
          <div>
            <p class="eyebrow" style="margin-bottom:var(--space-3)">Global rules</p>
...
export function pipelinesPage () {
  const body = wrap(`
    ...
    ${corpus.pipelines.map((p) => `
      <article class="panel" id="${attr(p.id)}" style="margin-top:var(--space-6)">
        <p class="mono muted" style="font-size:13px;margin:0 0 var(--space-2)">${esc(p.id)}</p>
        <h2 style="font-size:28px">${esc(p.name)}</h2>
  ...
  const body = wrap(`
    ...
    ${corpus.profiles.map((p) => `
      <article class="card" id="${attr(p.id)}">
        <p class="mono muted" style="font-size:13px;margin:0 0 var(--space-2)">${esc(p.id)}</p>
        <h2 style="font-size:24px">${esc(p.name)}</h2>
...
export function metricsPage () {
  ...
  const body = wrap(`
    ...
    <h2 style="margin-top:var(--space-9)">The twelve metrics</h2>
    <div class="table-scroll">
      <table>
        ...
        <tbody>${(m.metrics ?? []).map((x) => `
          <tr>
            <td style="color:var(--rp-heading);font-weight:600">${esc(x.id.replace(/_/g, ' '))}</td>
    ...
    <h2 style="margin-top:var(--space-9)">What a score means</h2>
    <dl class="deflist">${Object.entries(m.score_meanings ?? {}).map(([k, v]) => `
      <dt>${esc(k)}</dt><dd>${esc(String(v).replace(/_/g, ' '))}</dd>`).join('')}
    </dl>

    <h2 style="margin-top:var(--space-9)">Hard gates</h2>
    <div class="card">${conditionTree(m.hard_gates?.if)}
      <p style="margin:var(--space-4) 0 0">Then: <strong style="color:var(--rp-heading)">${esc(String(m.hard_gates?.then ?? '').replace(/_/g, ' '))}</strong></p>
      <p style="margin:var(--space-1) 0 0" class="muted">Else: ${esc(String(m.hard_gates?.else ?? '').replace(/_/g, ' '))}</p>
    </div>

    <h2 style="margin-top:var(--space-9)">Outcome tests</h2>
    ...
    <p style="margin-top:var(--space-9)"><a href="/api#score">Score a document over the API →</a></p>
  `)
...
export function sourcesPage () {
  ...
  const body = wrap(`
    ...
    <div class="panel"><p style="margin:0">${esc(corpus.byNum.get('29')?.json?.copyright_note ?? '')}</p></div>

    ${[...byType.entries()].map(([type, items]) => `
      <h2 style="margin-top:var(--space-9)">${esc(String(type).replace(/_/g, ' '))} <span class="muted" style="font-size:0.5em;font-weight:400">${items.length}</span></h2>
      <div class="table-scroll">
        <table>
          ...
          <tbody>${items.map((s) => {
            ...
            <tr id="${attr(s.id)}">
              ...
              <td style="color:var(--rp-heading)">${s.url ? `<a href="${attr(s.url)}" rel="noopener">${esc(s.title)}</a>` : esc(s.title)}${s.note ? `<br><span class="muted" style="font-size:13px">${esc(s.note)}</span>` : ''}</td>
              <td>${esc(s.creator ?? '')}</td>
...
export function glossaryPage () {
  ...
  const body = wrap(`
    ...
    <dl class="deflist" style="margin-top:var(--space-8)">${terms.map((t) => `
...
export function testsPage (run) {
  const body = wrap(`
    ...
    <div class="table-scroll" style="margin-top:var(--space-8)">
      <table>
        ...
        <tbody>${corpus.tests.map((t) => `
          <tr id="${attr(t.id)}">
            ...
            <td>${esc(t.assertion)}<br><span class="muted" style="font-size:13px">${esc(t.reason)}</span></td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>
...
export function filesPage () {
  ...
  const body = wrap(`
    ...
    <div class="card"><p style="margin:0"><strong style="color:var(--rp-heading)">Start here:</strong></p>
      ...
      <table>
        ...
        <tbody>${corpus.files.map((f) => `
          <tr>
            ...
            <td><a href="/files/${attr(f.slug)}" style="color:var(--rp-heading);font-weight:600">${esc(f.title)}</a><br><span class="mono muted" style="font-size:13px">${esc(f.filename)}</span></td>
...
export function filePage (entry) {
  ...
  const body = wrap(`
    ...
    <p class="mono muted" style="font-size:14px">${esc(entry.filename)} · version ${esc(entry.json.version ?? corpus.manifest.version)}</p>

    ${rulesHere.length ? `
      <h2 style="margin-top:var(--space-9)">The ${rulesHere.length} rules in this file</h2>
      <ol class="rules">${rulesHere.map(ruleRow).join('')}</ol>` : ''}

    <h2 style="margin-top:var(--space-9)">The file</h2>
...
export function searchPage ({ q, results }) {
  const body = wrap(`
    ...
    <form method="get" action="/search" class="panel" style="margin-top:var(--space-7)">
      <div class="field">
        ...
        <input type="search" id="q" name="q" value="${attr(q ?? '')}" aria-describedby="q-hint" autofocus>
      </div>
      <button class="pill pill--solid" type="submit" style="margin-top:var(--space-4)">Search</button>
    </form>

    ${q ? `
      <p class="muted" role="status" style="margin-top:var(--space-7)">${results.length} result${results.length === 1 ? '' : 's'} for “${esc(q)}”.</p>
      ${results.length ? `<ol class="rules">${results.map((r) => `
        <li><a class="rule" href="${attr(r.href)}">
          ...
          <span></span>
        </a></li>`).join('')}</ol>` : '<div class="card"><p style="margin:0">Nothing matched. Try fewer words.</p></div>'}
    ` : ''}
  `)
      ...
      ${[...grouped.entries()].map(([group, items]) => `
        ...
        <div class="table-scroll">
          <table>
            ...
            <tbody>${items.map((e) => `
              <tr>
                <td class="mono" style="color:var(--rp-heading)">${esc(e.method)}</td>
  ...
  <section id="facts">
    <div class="wrap stack">
      ...
      <div class="panel">
        ...
        <p style="margin:0;font-size:18px;color:var(--rp-heading)">
